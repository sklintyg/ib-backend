    ...
    public static GetBestallningResponse from(Utredning utredning, UtredningStatus utredningStatus, BusinessDaysBean businessDays) {
        ...
        return GetBestallningResponseBuilder.agetBestallningResponse()
                ...
                .withInkomDatum(utredning.getExternForfragan()
                        .map(ex -> nonNull(ex.getInkomDatum()) ? ex.getInkomDatum().format(formatter) : null)
                        .orElse(null))
                .withBesvarasSenastDatum(utredning.getExternForfragan()
                        .map(ex -> nonNull(ex.getBesvarasSenastDatum()) ? ex.getBesvarasSenastDatum().format(formatter) : null)
                        .orElse(null))
                ...
                .withIntygSistaDatumKomplettering(utredning.getIntygList()
                        .stream()
                        .filter(intyg -> intyg.isKomplettering() && intyg.getSistaDatum() != null)
